...
//macro for i2c0
//Посылка команды по I2C0
#define I2C0_CMD(CMD) I2C0->STCMD = CMD
//Проверка статусного регистра на установку бита прерываний
#define I2C0_INT_FLAG ((I2C0->STCMD & 0x01)? 1 : 0)
//Проверка статусного регистра на установку бита передачи данных, а также формирование сигнала STOP
#define I2C0_TIP ((I2C0->STCMD & 0x02)? 1 : 0)
//Проверка статусного регистра на установку бита потери контроля над шиной
#define I2C0_AL ((I2C0->STCMD & 0x32)? 1 : 0)
//Проверка статусного регистра на занятость шины, бит сбрасывается при получении сигнала STOP
#define I2C0_BUSY ((I2C0->STCMD & 0x64)? 1 : 0)
//Проверка статусного регистра на прием сигнала ACK
#define I2C0_ACK_RX ((I2C0->STCMD & 0x80)? 1 : 0)
//Прием байта по I2C0
#define I2C0_RX_DATA I2C0->DATA
//Посылка байта по I2C0
#define I2C0_TX_DATA(BYTE) I2C0->DATA = BYTE
//Разрешение прерываний по завершению передачи по I2C0
#define I2C0_EN_INT I2C0->CR |= 0x40 

//macro for i2c1
//Установка адреса ведомого устройства
#define I2C1_ADDR(ADDRESS) I2C1->ADDR = ADDRESS
//Проверка статусного регистра на передачу NACK(только при совпадении адреса) (0 - не передан;1 - передан)
#define I2C1_ST_SEND_NACK ((I2C1->ST & 0x01)? 1 : 0)
//Проверка статусного регистра на передачу байта данных (0 -  байт передан;1 - байт не передан)
#define I2C1_ST_SEND_BYTE ((I2C1->ST & 0x02)? 1 : 0)
//Проверка статусного регистра на прием байта данных (0 - байт не принят;1 - байт принят)
#define I2C1_ST_GET_BYTE ((I2C1->ST & 0x04)? 1 : 0)
//Очистка бита передачи байта данных статусного регистра
#define I2C1_CLEAR_SEND_BYTE I2C1->ST |= 2
//Очистка бита передачи NACK статусного регистра
...
//Чтение текущего адреса I2C1
#define I2C1_READ_ADDR I2C1->ADDR
//Прием байта по I2C1(чтение регистра приема)
#define I2C1_RX_DATA I2C1->RX
//Посылка байта по I2C1
#define I2C1_TX_DATA(BYTE) I2C1->TX = BYTE

//prototypes
//i2c0(master)
//Разрешение работы i2c0 и установка скорости работы интерфейса и системной частоты процессора
void i2c0_init(char enable, int clock_speed, int system_freq);
//Отправка байта данных по i2c0
void i2c0_send(char byte, char slv_addr, int addr);
//Прием байта данных по i2c0
char i2c0_get(char slv_addr, int addr);

//i2c1(slave)
//Настройка I2C1(slave) в соответствии с параметрами(пользователь самостоятельно конфигурирует порты ввода-вывода)
